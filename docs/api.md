...
`FORBIDDEN`, `PASSWORD_CHANGE_REQUIRED`, `NO_COOPERATIVE_ACCESS`, `NOT_FOUND`,
`DUPLICATE_RESOURCE`, `CONFLICT`, `INSUFFICIENT_STOCK`, `INVALID_STATE_TRANSITION`,
`IDEMPOTENCY_KEY_REUSED`, `FILE_TOO_LARGE`, `UNSUPPORTED_FILE_TYPE`, `RATE_LIMITED`,
`SERVICE_UNAVAILABLE`, `INTERNAL_ERROR`.

`PASSWORD_CHANGE_REQUIRED` (403) is answered to every request but `/auth/me`,
`/auth/change-password`, `/auth/sessions` and `/auth/logout` by an account that must still replace
a password somebody else chose for it. The interface holds such an account at
`/change-password`.
